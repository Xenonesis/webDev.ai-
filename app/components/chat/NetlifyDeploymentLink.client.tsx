...
export function NetlifyDeploymentLink() {
  ...
  const deployedSite = connection.stats?.sites?.find((site) => site.name.includes(`webdev-ai-${currentChatId}`));
      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <a
            ...
            title={`Open ${deployedSite.name} deployment`}
            aria-label={`Open ${deployedSite.name} deployment in new tab`}
