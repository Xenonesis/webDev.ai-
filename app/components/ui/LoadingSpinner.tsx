    ...
    >
      <div className="bg-white dark:bg-gray-900 rounded-xl p-6 shadow-xl border border-gray-200 dark:border-gray-700 max-w-sm mx-4">
        <div className="flex flex-col items-center space-y-4">
          ...
          {message && <p className="text-sm text-bolt-elements-textSecondary text-center">{message}</p>}
          {children}
        </div>
      </div>
    </div>
...
export function Skeleton({ className, variant = 'text', width, height, lines = 1 }: SkeletonProps) {
  ...
  if (variant === 'circular') {
    return <div className={classNames(baseClasses, 'rounded-full', className)} style={{ width, height }} />;
  }

  if (variant === 'rectangular') {
    return <div className={classNames(baseClasses, 'rounded-lg', className)} style={{ width, height }} />;
  }
  ...
  return (
    <div className={classNames('space-y-2', className)}>
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className={classNames(baseClasses, 'h-4 rounded', i === lines - 1 && lines > 1 ? 'w-3/4' : 'w-full')}
