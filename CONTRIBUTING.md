...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **webdev.ai** a better tool for developers worldwide. 💡
...
```bash
docker run -p 5173:5173 --env-file .env.local webdev-ai:development
```
...
```bash
docker build . --target webdev-ai-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local webdev-ai:production
```
